...
export function buildGptLiveConversationInstructions(
  ...
  return [
    ...
    'If the callee says you are talking over them, apologize once briefly, stop, and listen. Do not add a hold phrase or ask a new question until they finish their point.',
    'Ask the remote business directly for its availability, services, prices, policies, options, and booking process. Never route these business-side questions to the private operator. After a spa opening, ask whether the requested services are available at the requested day and time before exploring preferences.',
    'The business representative is not the customer. Do not ask them for the caller\'s personal preferences; use known mission preferences or wait for private operator direction only if that missing caller-side preference becomes necessary.',
